...
def get_compareTitleCard(data): 
    ...
    if default is None: default = SQUAD_DEFAULT 
    ...
    return dbc.Card([
        ...
        [
            dbc.Row([
                dbc.Col([
                    html.Center(html.H5("ColBERT vs Elastic", style={"font-family":"Quicksand", "color":TEAL,'font-size':"40px", "padding":"0.5rem"})),
                ], width=5), 
                dbc.Col([                
                     ...
                     dbc.Row([
                        dbc.Col([
                            ...
                            (
                                    ...
                                    style={"font-family":"Quicksand", "color":WHITE, 'font-size':"22px","padding":"0rem"}
                            ),
                        ], width=4), 
                        dbc.Col([
                            dbc.Select(
                        ...
                        dbc.Col([
                        html.Center(dbc.Button(f'Begin Benchmark', color="info", outline=True, id="compare-begin-index", style={'font-size':"26px",'font-family':"Quicksand"}))          
                ]),
...
def get_compareMetricsCard(data, pQueue, m_name): 
            ...
            [
                html.Center(html.H6
                        (
                                ...
                                style={"font-family":"Quicksand", "color":CREAM, 'font-size':"22px"}
                        ), 
    ...
    try: 
        results = pQueue.get_nowait(); print("1")
            ...
            [
                ...
                dbc.Col([
                    ...
                    ([
                        ...
                        (
                                "Num. Correct (#):", 
                                style={"font-family":"Quicksand", "color":WHITE, 'font-size':"22px"}    
                        ), width=4),
                        dbc.Col(dbc.Badge(html.H1(num_correct, style={"font-family":"Quicksand", 'font-size':"22px"}), color="dark", text_color="primary")),
                        ...
                        dbc.Col(dbc.Badge(html.H1(num_total, style={"font-family":"Quicksand", 'font-size':"22px"}), color="dark", text_color="primary")),
                    ...
                    ([
                        ...
                        dbc.Col(dbc.Badge(html.H1(avg_time, style={"font-family":"Quicksand", 'font-size':"22px"}), color="dark", text_color="primary")),
                    ]),
                ]),
            ])
    ...
    for wrong in incorrect: 
        ...
        accordion_list.append(
            dbc.AccordionItem(
                ...
                style={'font-family':"Quicksand", 'color':WHITE, 'background-color':"#049FD911", 'text-color':WHITE}
            )
        )
...
def get_compare_spinnyCircle(): 
      return dbc.Card([
            html.Div(
            [
                html.Center(dbc.Spinner(color="info", spinner_style={"width": "3rem", "height": "3rem"}), style={"margin-top":"10rem"})
            ])
...
def get_squadTitleCard(data): 
    ...
    return dbc.Card([
        html.Div(
        [
            html.Center(html.H5(default, style={"font-family":"Quicksand", "color":CREAM,'font-size':"30px", "padding":"1rem"})),
            dbc.Row([
                        dbc.Col([
                            html.H6
                            (
            ...
            html.Center(dbc.Button(f'Begin {data["model"]["name"]} Benchmark', color="info", outline=True, id="squad-begin-index", style={'font-size':"26px",'font-family':"Quicksand"}))          
...
def get_squadMetricsCard(data, pQueue): 
    ...
    percent_correct = round(100 * np.mean(results["metrics"]["correct_arr"]), 2)
    num_correct = results["metrics"]["correct_arr"].count(1)
    num_total = results["questions"]["num_qs"]
    num_curr = results["questions"]["tot_qs"]
    avg_time = round(np.mean(results["times"]["all_ts"]), 2)
            ...
            [
                ...
                dbc.Col([
                    ...
                    ([
                        ...
                        dbc.Col(dbc.Badge(html.H1(percent_correct, style={"font-family":"Quicksand", 'font-size':"22px"}), color="dark", text_color="primary")),
                    ]),
                    html.Br(),
                    dbc.Row
                    ([
...
def gen_inputOptions(files): 
    ...
    for i in range(len(files['n_user'])): 
        options.append({"label": files['n_user'][i], "value": files['p_user'][i]})


    for i in range(len(files['n_squad'])): 
        options.append({"label": files['n_squad'][i], "value": files['p_squad'][i]})
...
def initialize_data(): 
    ...
    n_squad, p_squad = [], [] 
    datasets = [dir[0] for dir in os.walk(SQUAD_DATA_PATH)]
    ...
    n_user, p_user = [], [] 
    datasets = [dir for dir in os.listdir(FILES_DATA_PATH)]

    for dir in datasets: 
        n_user.append(dir)
        p_user.append(FILES_DATA_PATH + "/" + dir)
...
def get_spinnyCircle(): 
      return dbc.Card([
            html.Div(
            [
                html.Center(dbc.Spinner(color="info", spinner_style={"width": "3rem", "height": "3rem"}), style={"margin-top":"50%"})
            ])
        ], outline=True, color="#049FD911", style={"color":"dark","padding":"1rem", 'font-family':"Quicksand", "height":"32rem", "vertical-align":"middle"}
        ) 
