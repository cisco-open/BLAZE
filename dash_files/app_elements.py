...
def get_sidebar(data):
    ...
    sidebar = html.Div([
        ...
        html.P("""Kindley select a retriever model 
                ...
                [
                    dbc.RadioItems(
                        options=[
                            ...
                            {"label": "ColBERT", "value": 1, "disabled": True},
                            {"label": "Elastic", "value": 2},
                            # Currently implementing
                            {"label": "K-Graph", "value": 3, "disabled": True},
                        ],
                        value=2,
        ...
        dbc.Card([
            html.Div(
                [
                    dcc.Upload(dbc.Button('Upload File (.txt, .pdf)', color="info", outline=True, style={
                               'font-family': "Quicksand"}), id="input-button-file"),
                ]),
...
def get_inputBox(data):
    ...
    if data['states']['has_input_file']:
        preview, file_txt = gen_filePreview(
            data['states']['chosen_name'], data['states']['chosen_path'])
    ...
    inputBox = dbc.Card([
        dbc.CardBody([
                        ...
                        html.Center(dbc.Button(f'Begin {data["model"]["name"]} Indexing', color="info", outline=True,
                                    id="input-indexing", style={'font-size': "26px", 'font-family': "Quicksand"}))
                        ],
                     className="mb-3",
                    ...
                    [
                        dbc.Row([
                            ...
                            dbc.Col(dbc.Button('Ask Q', color="info", outline=True,
                                               id="input-qbutton", style={'font-family': "Quicksand"}))
                        ]),
                        dbc.Alert(
    ...
    accuracyCard = dbc.Card([
        html.Div(
            [
                ...
                table2,
                html.Br(),
                html.Center(html.H5(html.A(f"Learn more about {model['name']}.", href=model['l_info'], target="_blank", style={
                            "font-family": "Quicksand", "color": WHITE}))),
                html.Center(html.H5(html.A(f"View {model['name']} Github Repo.", href=model['l_repo'], target="_blank", style={
                            "font-family": "Quicksand", "color": WHITE})))
