...
class ColbertSearch: 
    ...
    def __init__(self, file_name, file_content):
        ...
        with Run().context(RunConfig(nranks=1, experiment=self.experiment)):
            ...
            self.indexer = Indexer(checkpoint=self.checkpoint, config=self.config)
            self.indexer.index(name=self.index_name, collection=self.docs, overwrite=True)  
    ...
    def file_search(self, search_term):
        ...
        for i, _, _ in zip(*docs):
            ...
            r, start, end = answer_question(search_term, c_d, self.model, self.tokenizer)
            if r == "": continue
