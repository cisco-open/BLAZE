...
class SummaryTestcase(unittest.TestCase):
    ...
    def test_all_datasets(self):
        response = self.client.get('/files/all_datasets')
    ...
    def test_all_models(self):
        response = self.client.get('/models/all_models', json={})
        assert all(key in response.json for key in [
                   "models_search", "models_summarization"])

    def test_all_files(self):
        response = self.client.get('/files/all_files', json={
            "dataset": "User",
        })
        assert all(key in response.json for key in ["files"])

    def test_summary(self):
        response = self.client.get('/models/summary', json={
            "model": "Bart",
