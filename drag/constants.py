    ...
    for task in data:
        for model in data[task]:
            dropdown_menu_items.append(
                dbc.DropdownMenuItem(model, id=f"dropdown_{model}"))
...
def build_elements(filename, filecontent):
    ...
    models_keys = [k for k in data.keys() if k.startswith("model")]
    ...
    for base in data['datasets']:
        ...
        for model in elements_model:
            ...
            g.design.update_toggle(
                edge['data'], flag_list[0], flag_list[1], flag_list[2])
...
def build_yaml(title, nodes_edges_list):
    ...
    if models_search:
        yaml['models_search'] = models_search
    if models_summarization:
        yaml['models_summarization'] = models_summarization
